...
pub const INTERROGATION_SYSTEM_PROMPT: &str = r#"You are EchoMind's "Soul Interrogation" AI assistant. Your goal is to help the user sharpen a vague idea into something concrete.

Your interrogation framework:
1. What problem does this idea solve? Who has this problem?
2. How do people solve it today? Why isn't that good enough?
3. How does your solution differ from existing alternatives?
4. Where's the moat? Why can't others easily copy it?
5. How big is the market? Is it worth doing?

Rules:
- Ask only 1-2 questions at a time — don't fire them all at once
- Progress based on the user's answers; don't repeat questions already answered
- If the user's answer exposes a logical hole, call it out directly
- Friendly but sharp tone, like an experienced startup mentor
...
impl EchoMind {
    ...
    pub async fn enrich_thought(&self, thought_id: &str) -> Result<Thought, String> {
        ...
        let system_prompt = r#"You are EchoMind's AI assistant. The user just captured an idea, possibly with an attached file or image.

Analyze the content and return JSON:
{
  "context": "one-sentence statement of background and intent (under 40 chars)",
  "domain": "pick one English lowercase word from: technology / science / design / business / personal / creative / philosophy / health / education / finance / other",
  "tags": "keyword1,keyword2",
  "file_summary": "if a file or image is attached, summarize its core content in 2-3 sentences; otherwise empty string"
}

Strict rules:
- Return JSON only — no extra text, comments or markdown fences
- Do NOT use ASCII double quotes inside JSON string values. If quoting is needed, use single quotes '
- Do not include newlines inside string values
- The "context" / "tags" / "file_summary" values should be written in the same language as the user's idea
- The "domain" field MUST be exactly one of the 11 English words above — never translate or invent a new one"#;
    ...
    pub async fn synthesize_chat_plan(&self, conversation_id: &str) -> Result<String, String> {
        ...
        let system = r#"[Language]: Detect the language of the user's conversation messages and write the entire plan in that same language. The Chinese section headings shown later (a Chinese H2 like 一句话定位) are templates — translate them to the user's language naturally (e.g. One-line Positioning for English). Instruction tags such as 常识 / 估算 should likewise be localized (common knowledge / estimate). Output structure and rules stay the same.

你是资深的产品策略 + 可行性分析师。用户会给你一段他和 AI 的拷问/讨论对话，请把它蒸馏为一份**结构完整、可交付给协作者或决策者**的可行性分析报告。
...
记住目标：一份**让没看过对话的协作者也能基于这份报告判断是否值得投入**的文档。详细度上对标"投资决策包"，不是"会议纪要"。"#;
    ...
    pub async fn suggest_resources(&self, thought_id: &str) -> Result<Vec<serde_json::Value>, String> {
        let (thought, chat_context, tavily_key, provider_type, config) = {
            let conn = self.conn()?;
            let thought = thoughts::get_thought(&conn, thought_id).map_err(|e| e.to_string())?;
            ...
            let tavily_key = settings::get_setting(&conn, "web_search_api_key")
                .map_err(|e| e.to_string())?
                .map(|s| s.trim().to_string())
                .filter(|s| !s.is_empty());

            let (pt, cfg) = Self::load_llm_config_from_conn(&conn)?;
            (thought, chat_context, tavily_key, pt, cfg)
        };
        ...
        // If Tavily is configured: run a real web search first, then let the
        // LLM choose from the live candidates. This is the "grounded" path —
        // 0% URL hallucination because the model is *constrained* to pick
        // from URLs the search returned. Falls back to the legacy LLM-only
        // path if no key is set, so existing setups keep working.
        let messages = if let Some(key) = tavily_key.as_ref() {
            let mut query = thought.content.chars().take(200).collect::<String>();
            if let Some(d) = thought.domain.as_ref() {
                query.push(' ');
                query.push_str(d);
            }
            if let Some(t) = thought.tags.as_ref() {
                query.push(' ');
                query.push_str(t);
            }
            let candidates = web_search::tavily_search(key, &query, 10)
                .await
            ...
            for (i, c) in candidates.iter().enumerate() {
                let snippet = c.content.chars().take(220).collect::<String>();
            ...
            vec![
                ChatMessage {
                    role: "system".to_string(),
                    content: r#"You are a resource recommendation assistant. I've already run a web search for this idea, and the candidates below are real, accessible resources.

Your task:
1. Pick up to 6 (at least 3) candidates **most relevant** to the idea's theme.
2. For each, write one sentence explaining **why it relates to this idea** (do not paraphrase the summary).
3. Pick a suitable type for each: article / tool / doc / project / book / course.

**Strict rules**:
- You may ONLY use URLs that appeared in the candidates; **NEVER** invent a new URL.
- If none of the candidates fit, return fewer items — even an empty array.
- The title may be lightly adjusted (simplified, dropping site-name suffix), but don't change its meaning.
- Write title + description in the same language as the idea's content.

Return a JSON array, **nothing else**:
[
  { "title": "...", "url": "...", "type": "article|tool|doc|project|book|course", "description": "..." }
]"#.to_string(),
                },
                ChatMessage {
                    role: "user".to_string(),
                    content: format!("候选资源：\n{}\n灵感信息：\n{}", candidate_block, content_desc),
                },
            ]
        } else {
            vec![
                ChatMessage {
                    role: "system".to_string(),
                    content: r#"You are a resource recommendation assistant. Based on the user's full idea context (content, file summary, tags, discussion), recommend 4-6 real online resources directly relevant to the idea's theme.

Key: carefully read all the provided information, identify the idea's core theme; recommended resources must tie tightly to that theme.

Resource types can include: websites, articles, tools, open-source projects, docs, books, courses, etc.

Requirements:
- Only recommend well-known resources you are confident actually exist
- The URL must be a real, reachable main domain or known page path
- Resources must relate directly to the idea's core theme — don't pad with generic tools
- Resources may be in any language; pick whatever fits the theme best
- Write title + description in the same language as the idea's content
...
[
  {
    ...
    "description": "one-sentence explanation of how this resource relates to the idea"
  }
]

Return ONLY the JSON array, nothing else."#.to_string(),
                },
                ChatMessage {
                    role: "user".to_string(),
    ...
    /// Validate the stored Tavily key by issuing a tiny live search.
    /// Returns a short success summary on 2xx; surfaces Tavily's error
    /// message verbatim on failure (rate-limit / invalid-key / network).
    pub async fn test_web_search(&self) -> Result<String, String> {
        let key = {
            let conn = self.conn()?;
            settings::get_setting(&conn, "web_search_api_key")
                .map_err(|e| e.to_string())?
                .map(|s| s.trim().to_string())
                .filter(|s| !s.is_empty())
                .ok_or_else(|| "未配置 API Key".to_string())?
        };
        let results = web_search::tavily_search(&key, "EchoMind sqlite vector search", 3).await?;
        if results.is_empty() {
            Ok("连接成功，但未返回结果（key 有效）".to_string())
        } else {
            Ok(format!(
                "✓ 连接成功 · 返回 {} 条结果 · 首条：{}",
                results.len(),
    ...
    pub async fn summarize_thoughts(&self, ids: &[String]) -> Result<String, String> {
        ...
        let messages = vec![
            ChatMessage {
                role: "system".to_string(),
                content: "You are an idea-organizing assistant. Take the multiple note entries the user provides and condense them into a Markdown summary: first one sentence distilling the shared theme, then 3-5 bullet points listing the relations or contrasts between the ideas, and finally one actionable suggestion. Concise tone, lists allowed. Respond in the same language as the user's notes.".to_string(),
            },
            ChatMessage {
                role: "user".to_string(),
