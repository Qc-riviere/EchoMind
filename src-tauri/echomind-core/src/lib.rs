    ...
    pub async fn suggest_resources(&self, thought_id: &str) -> Result<Vec<serde_json::Value>, String> {
        ...
        let messages = if let Some(key) = tavily_key.as_ref() {
            ...
            vec![
                ChatMessage {
                    role: "system".to_string(),
                    content: r#"你是一个资源推荐助手。我已经为这个灵感做过网页搜索，下面是真实存在、可访问的候选资源。

你的任务：
1. 从候选里挑出最多 6 条（最少 3 条）与灵感主题**最相关**的资源。
2. 给每条写一句话，说明它**为什么和这条灵感相关**（不要复述摘要）。
3. 给每条选一个合适的 type：article / tool / doc / project / book / course。

**严格规则**：
- 只能使用候选里出现过的 URL；**绝对不要**自己写新的 URL。
- 如果候选都不相关，宁可返回更少的条数；甚至返回空数组。
- title 可以基于候选 title 微调（简化、去掉网站名后缀），但不要篡改大意。
