...
- **2026-06-22 (v0.3.8-dev)** | N3 置顶多选 + N4 开机自启 + N5 微信桥静默启动 + getting-started 英文版 | 用户一批 4 个体验请求（N3–N6），完成前 3 + 文档；N6（VPS 对外测试 pair codes）属敏感 VPS 操作，待授权。**(N5)** `src-tauri/src/commands/bridge_cmds.rs` sidecar spawn（bridge-server + wechat daemon 两路）在 Windows 经新 `hide_console_window()` helper 加 `CREATE_NO_WINDOW`(0x08000000)——首次启动微信桥不再弹黑色 terminal 窗。**(N4)** 开机自启动，默认关；`setup` 检测到 `--minimized` arg 就把 `main` 窗 `.hide()` 只起系统托盘不弹主窗；SettingsPage 新建 **General** tab + 自启开关（读 OS 真实注册态而非本地 setting）。**实现踩坑**：最初用 `tauri-plugin-autostart`，但它依赖的 `auto-launch-0.5.0`（windows.rs:42）把 Run 值写成 `format!("{} {}", exe_path, args)`——**exe 路径不加引号**，凡路径含空格（dev 的 `D:\File VS code\…` 和生产的 `C:\Program Files\EchoMind\…` 都中招）Windows 开机解析不了，静默不启动（这就是用户报的"开机启动没成功"）。改为 Windows 端自己用 `winreg` 直接读写 `HKCU\…\Run`，值用 `"{exe}" --minimized`（引号包路径）；新增 `autostart_cmds.rs`（命令 `autostart_is_enabled` / `autostart_set_enabled`，非 Windows 仍委托插件的 `AutoLaunchManager`，其 mac/linux 路径无此 bug）+ `setup` 启动时 `resync_windows_entry()` 自愈旧的无引号 Run 项 / 更新后路径漂移；前端 GeneralTab 改调这俩 invoke 命令、不再用 `@tauri-apps/plugin-autostart` JS API。插件 Rust 端仍注册（供非 Windows 委托用）。**(N3)** 置顶从单条扩到多条（上限 5）：`echomind-core` 加 `pin_order` 列 + init.rs 迁移；`get_pinned_thought`→`get_pinned_thoughts`（返 Vec，`ORDER BY pin_order ASC` 即新置顶在最前）；`set_pinned` 加 cap 检查（满 5 返回 `PIN_LIMIT:5` 的 SqliteFailure）+ 新置顶取当前 `min(pin_order)-1` 排最前；新增 `reorder_pinned` 事务按 id 数组重写顺序；`HomeThoughts.pinned` Option→Vec、`list_home_thoughts` 改用 HashSet 去重 recent；新 Tauri 命令 `reorder_pinned_thoughts`。前端 `HomePage` 新增 `PinnedSection`：grip 手柄拖拽（仅手柄 `draggable`，卡片文字仍可选、按钮可点）、`setDragImage(整卡)` 让整条灵感跟随光标、被拖原卡 `opacity-40 blur` 虚化、`dragOver` 实时 live-reorder + 手写 FLIP（记录旧 top → invert translateY → 180ms 缓动）做"挤压让位"动画、置顶框加 `pt-2 px-2 pb-4` 包住背后 peek 折角不溢出边框；`ThoughtCard` 超额时 toast `thought.pin_limit`（zh/en）；新增回归测试 `echomind-core/tests/pinning.rs` 4 项（cap5 / 新置顶置顶 / 解除释放槽位 / 手排持久化）；README zh/en 的「置顶」描述由"单条"改为"最多 5 条 + 拖拽手排"。**关键坑（拖拽全程拖不动）**：Tauri webview 默认 `dragDropEnabled:true` 会拦截 OS 层拖放，导致 HTML5 `dragstart`/`drop` 在 JS 里根本不触发；`tauri.conf.json` 主窗设 `dragDropEnabled:false` 同时修好了置顶拖拽 + ThoughtInput 拖图片落库（后者读 `dataTransfer.files`，之前也被 Tauri 吞掉）。**(文档)** 新建 `workspace/public_repo/docs/getting-started.en.md`（9 节完整英译，术语对齐 README.en：capture overlay / WeChat Bridge / Cloud Bridge / mind-graph / L2 Bridge）+ 中英互链 + 修 public `README.en.md` 文档链接指向 `.en`。**验证**：`pnpm tsc` / `cargo check` / `cargo test -p echomind-core` 全过；提交于 `51bf90d`（功能）+ `ac81a8c`（文档），已 push master、CI 绿。人工 e2e 见下条 2026-06-26。 |
- **2026-06-26 (v0.3.7 pkg)** | N4 自启打包 e2e + 图标字体脱离 CDN | 用 `pnpm tauri build` 出安装版真机验 N4/N5：开机自启注册表项确认正确——`"D:\EM\EchoMind\echomind.exe" --minimized`（**带引号**，对照修复前那条无引号、指向 dev `target\debug` 的坏项），重启后只起系统托盘、不弹主窗、不弹黑窗、无 localhost 报错，N4/N5 e2e 通过。**但首启暴露一个独立老 bug**：所有图标渲染成连字名文本（home / search / push_pin / drag_indicator…）。根因：图标字体 Material Symbols Outlined 一直从 Google Fonts CDN 加载（`index.html` 的 `<link href="fonts.googleapis.com/css2?family=Material+Symbols+Outlined…">`），Google 的 css2 响应同时提供 `@font-face` + `.material-symbols-outlined` 基类（`font-family` + `font-feature-settings:liga` 连字），`App.css` 只额外叠 `font-variation-settings`。平时手开 app 网络已就绪故看不出；**开机自启时网络未就绪 → CDN 拉取失败 → 每个图标 fallback 成 ligature 名**。桌面 app 不该依赖 CDN 字体。修：`pnpm add material-symbols`（marella 包 0.45.4）；`main.tsx` `import "material-symbols/outlined.css"`（放共享入口，主窗 + 捕获窗都覆盖；该包 css 是 CDN 的 drop-in，同样给 `@font-face`+基类，`font-display:block` 避免连字名闪现）；删 `index.html` 的 CDN link。Vite 把 3.96MB woff2 打进 `dist/assets/`，安装包随之 +4MB（49.5→53.5MB）。验证：重装重启后图标全部正常离线渲染。 |
