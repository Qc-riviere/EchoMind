...
| 对话导出 MD/DOCX/PDF | ✅ | 任意对话可一键导出 Markdown / Word / PDF（浏览器打印），用于二次整理或分享 |
| 思维图谱 | ✅ | 全部灵感按语义相似度形成网络图，按 domain 着色 + 近 24h 标记 + 暗色主题适配 |
| 多 LLM 提供商 | ✅ | OpenAI / Claude / Gemini / DeepSeek / 智谱 等可切换；BYO Key；非 OpenAI/Gemini 自动 fallback 到本地 bge-small-zh-v1.5 做向量检索 |
...
> **关于「8 轮上限」的澄清**：这 8 是**单条用户消息内 agent 调工具的迭代上限**，不是对话轮数。对话本身**无上限**——每条新用户消息开一轮，历史 messages 全带，瓶颈只在 LLM 的上下文窗口（GPT-4o ≈ 128K、Claude Sonnet ≈ 200K、DeepSeek ≈ 64K）。
>
> 真正受 8 限制的是：用户问"我之前写过关于 X 的想法吗？"后，模型可以"先 `search_thoughts` → 看结果不够细 → 再 `list_recent_thoughts` 补 → 再 `get_thought` 取详情 → 答你"这样链式调工具，**每环一次 LLM call，最多 8 环**。模型一旦不再 emit tool_call 就立即返回最终文字，所以日常对话通常一两环就结束。设 8 是为了平衡能力与失控成本：每环 1-3K tokens，硬上限避免模型死循环烧用户的 BYO Key 预算。
...
EchoMind 严格执行 Obsidian Sync 风格的「**明确代价 + 知情同意**」隐私模型。我们不假装零知识，但每个数据流向对用户透明、可关闭、可回滚。

### 3.1 数据流向矩阵

| 数据类别 | 默认存储位置 | 上云路径 | 加密 | 谁能读到 |
|---|---|---|---|---|
| **灵感原文** | 本地 SQLite（**唯一权威副本**） | 仅当用户主动开启「上云子集」并配置规则时，匹配的条目 AES-256-GCM 加密后传给用户自有 VPS | ✅ AES-256-GCM | 用户本人 + 用户自有 VPS |
| **向量索引**（sqlite-vec） | 本地 | **永不上云** | n/a | 用户本人 |
| **对话历史** | 本地 SQLite | **永不上云**（除非用户主动 MD/DOCX/PDF 导出） | n/a | 用户本人 |
| **LLM API Key** | 系统 keyring（默认） | 用户明确开启「云端转发」时推给 VPS，VPS 代调用 LLM | TLS in-transit | 用户本人 + 用户自有 VPS |
| **微信消息** | 经腾讯 ClawBot iLink 服务器中转（协议天然） | **必然过腾讯**（与日常微信消息相同） | TLS（iLink） | 用户本人 + 腾讯（与全部微信消息一样） |
| **设备配对码 / Device ID** | VPS SQLite | 单租户 VPS，不跨用户 | TLS + JWT 鉴权 | 用户本人 |
| **使用遥测 / 错误日志** | — | **不存在**：app 不收集 telemetry | n/a | 无 |

### 3.2 用户掌控的 5 个开关

1. **是否启用 Cloud Bridge** —— 关 = L1 Core 永远 100% 本地，没有任何数据外发
2. **上云子集规则** —— SQL-like 规则（如 `domain == "工作"`），用户决定哪些灵感会进 VPS；不匹配的永不上云
3. **是否把 LLM Key 推给 VPS** —— 不推 = 桌面端本地直连 LLM；推 = VPS 代为调用（用于离线时微信桥也能 `/chat`）
4. **VPS 预算硬上限** —— 默认 $1/天，VPS 端原子停用，防 Key 被劫持烧钱
5. **微信 ClawBot 解绑** —— 用户随时停用 bot 账号；桌面端继续独立运行

### 3.3 我们 NOT 做的 5 件事

- ❌ 不收集任何匿名遥测（无 Mixpanel / Sentry / Google Analytics）
- ❌ 不运营自有 LLM 端点 —— 全部 BYO Key，平台不承担也不掌握 token
- ❌ 不跨用户混放数据 —— 用户自带 VPS，单租户隔离
- ❌ 不做云端"自动备份"（避免用户误以为数据安全在云上，本地才是权威副本）
- ❌ 不读取或用用户数据训练模型

### 3.4 与 Obsidian Sync 的隐私模型对比

| | Obsidian Sync | EchoMind |
|---|---|---|
| 客户端 → 云端 | 端到端加密（仅用户能解密） | 子集 AES-256-GCM 加密 |
| 服务器在哪 | Obsidian 官方服务器（多租户） | 用户自有 VPS（境内 / 境外用户自选，单租户） |
| 隐私叙事 | "零知识" | "**明确代价 + 用户选范围**" |
| 永久免费部分 | 完整本地 PKM | 完整本地灵感系统 + 本地向量检索 + AI enrich（用户自带 Key） |
| 付费扩展 | Sync ¥4-10/月（多端同步） | Bridge ¥20-30/月（微信桥 + 云端 LLM 转发） |
| LLM 政策 | 无内置 AI | BYO Key，平台不接触 token |

### 3.5 一句话隐私承诺

> **关掉 Cloud Bridge，EchoMind 等同于一个加了 AI 自动补全的本地笔记本——所有数据，包括 AI 调用记录，永不出本机。开启 Cloud Bridge 后，每一个上云的子集都是用户主动选择的，加密后存在用户自己买的 VPS 上。**
...
### 9.1 阶段一：Closed Alpha（10-15 人，2-4 周）
...
### 9.2 阶段二：Friends & Family（30-50 人，4-8 周）
...
### 9.3 阶段三：Open Beta（100-300 人，8-12 周）
